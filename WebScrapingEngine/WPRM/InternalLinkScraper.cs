            ...
            foreach (var node in html.DocumentNode.SelectNodes("//a"))
            {
                try
                {
                    ...
                    if (link != null && this.IsInternalDomain(link.Value))
                    {
                        ...
                        if (!url.FullUrl.Contains('#') && !url.FullUrl.Contains('?') && !url.FullUrl.Contains(".jpg") && !url.FullUrl.Contains(".pdf") && !url.FullUrl.Contains(".jpeg") && !this.History.CheckUrl(url.FullUrl))
                        {
                            list.Add(url);
